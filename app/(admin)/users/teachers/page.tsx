...
export default function TeachersPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await listTeachers({ status: filter, limit: 100 });
  ...
  function handleApprove(profileId: string) {
    startTransition(async () => {
      try {
        const updated = await approveTeacher(profileId);
        setTeachers((prev) => prev.map((t) => t._id === profileId ? { ...t, profileStatus: updated.profileStatus } : t));
      } catch (err) {
  ...
  function handleRejectConfirm() {
    ...
    startTransition(async () => {
      try {
        const updated = await rejectTeacher(rejectTarget.id, rejectReason);
        setTeachers((prev) => prev.map((t) => t._id === rejectTarget.id ? { ...t, profileStatus: updated.profileStatus } : t));
  ...
  const filtered = teachers.filter((t) => {
    const name = t.personal?.fullNameEn ?? t.personal?.fullNameAr ?? "";
    return name.toLowerCase().includes(search.toLowerCase());
  });

  const pending   = teachers.filter((t) => t.profileStatus === "pending").length;
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl flex items-center justify-center shadow-sm" style={{ background: "linear-gradient(135deg, #0D2542, #1a3d6b)" }}>
              <GraduationCap className="h-5 w-5 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">Teachers</h1>
              <p className="text-xs text-slate-400 mt-0.5">Verify identities, review documents, award trust badges</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Total Teachers", value: total, icon: Users, gradient: "linear-gradient(135deg, #0D2542, #444882)", accent: "#0D2542" },
          { label: "Pending Verification", value: pending, icon: AlertCircle, gradient: "linear-gradient(135deg, #f59e0b, #f97316)", accent: "#f59e0b" },
          { label: "Approved", value: approved, icon: CheckCircle2, gradient: "linear-gradient(135deg, #24BFBF, #00ACD3)", accent: "#24BFBF" },
        ...
        <Table>
          ...
          <TableBody>
            {loading
              ...
              : filtered.map((t) => {
                  const name = t.personal?.fullNameEn ?? t.personal?.fullNameAr ?? "Unknown";
                  ...
                  return (
                    <TableRow key={t._id} className="border-slate-100 hover:bg-slate-50/50 transition-colors">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8">
                            <AvatarFallback className="text-[10px] font-bold text-white" style={{ background: "linear-gradient(135deg, #0D2542, #444882)" }}>{initials}</AvatarFallback>
                          </Avatar>
                          <div>
                      ...
                      <TableCell className="text-sm text-slate-600 capitalize">{subjects || "—"}</TableCell>
                      <TableCell className="text-sm text-slate-600">{t.professional?.experienceRange ?? "—"}</TableCell>
                      <TableCell className="text-sm text-slate-600 capitalize">{city}</TableCell>
                      <TableCell><StatusBadge status={t.profileStatus} /></TableCell>
                      <TableCell className="text-xs text-slate-400 tabular-nums">
                        {t.submittedAt ? new Date(t.submittedAt).toLocaleDateString("en-SA") : "—"}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => router.push(`/users/teachers/${t._id}`)}
                          ...
                          {t.profileStatus === "pending" && (
                            <>
                              ...
                              <button
                                onClick={() => setRejectTarget({ id: t._id, name })}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Provide a reason for rejecting <strong>{rejectTarget?.name}</strong>. This will be visible to the user.
            </AlertDialogDescription>
          </AlertDialogHeader>
